...
class ReIDDataset(BaseDataset):
    ...
    def evaluate(self,
        ...
        keep_indices = [i for i in range(0, len(features), 10)]
        ...
        pids = self.get_gt_labels()[keep_indices]
        ...
        for q_idx in range(n):
            distmat = torch.pow(features - features[q_idx: q_idx + 1], 2).sum(dim=1).numpy()
            indices = np.argsort(distmat)
            matches = (pids[indices] == pids[q_idx]).astype(np.int32)
