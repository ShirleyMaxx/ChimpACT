...
class MotChallenge2DBox(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            ...
            'CLASSES_TO_EVAL': ['chimpanzee'],  # Valid: ['chimpanzee']
    ...
    def __init__(self, config=None):
        ...
        # Get classes to eval
        self.valid_classes = ['chimpanzee']
        self.class_list = [cls.lower() if cls.lower() in self.valid_classes else None
                           for cls in self.config['CLASSES_TO_EVAL']]
        if not all(self.class_list):
            raise TrackEvalException('Attempted to evaluate an invalid class. Only chimpanzee class is valid.')
        self.class_name_to_class_id = {'chimpanzee': 0, 'background': 1}
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        MOT Challenge:
            In MOT Challenge, the 4 preproc steps are as follow:
                1) There is only one class (chimpanzee) to be evaluated, but all other classes are used for preproc.
                ...
                4) All gt dets except chimpanzee are removed, also removes chimpanzee gt dets marked with zero_marked.
        """
        # Check that input data has unique ids
        self._check_unique_ids(raw_data)

        distractor_class_names = ['background']
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            # Evaluation is ONLY valid for chimpanzee class
            if len(tracker_classes) > 0 and np.max(tracker_classes) > 1:
                raise TrackEvalException(
                    'Evaluation is only valid for chimpanzee class. Non chimpanzee class (%i) found in sequence %s at '
            ...
            # Remove gt detections marked as to remove (zero marked), and also remove gt detections not in chimpanzee
