...
class SampleAVAFrames(SampleFrames):
    ...
    def transform(self, results):
        ...
        center_index = fps * (timestamp - timestamp_start)
        ...
        frame_inds = np.array(frame_inds, dtype=np.int32)
        ...
        # results['offset'] = 0
        # import sys
        # print(results['offset'], flush=True)
        # sys.stdout.flush()
        # assert 0
        # if 0 == shot_info[0] and 571 == shot_info[1]:
        #     print(frame_inds)
        #     sys.stdout.flush()
        return results
...
class RawFrameDecode(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        for i, frame_idx in enumerate(results['frame_inds']):
            ...
            frame_idx += offset
            frame_idx = np.clip(frame_idx, results['shot_info'][0], results['shot_info'][1] - 1)
            if modality == 'RGB':
                filepath = osp.join(directory, filename_tmpl.format(frame_idx))
                # import sys
                # if not os.path.isfile(filepath):
                #     print(results['shot_info'], frame_idx)
                #     sys.stdout.flush()
